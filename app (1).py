...
def load_and_train():
    ...
    # ── PYCARET: find the best model automatically ──────────────────────────
    train_df = X_train.copy()
    train_df['Fraud_Label_01'] = y_train.values

    clf_setup = setup(
        data=train_df,
        target='Fraud_Label_01',
        session_id=42,
        fix_imbalance=True,
        normalize=True,
        verbose=False,
    ...
    results_df = pull()   # dataframe of compare_models results

    # ── Score the test set ──────────────────────────────────────────────────
    test_input = X_test.copy()
    preds = predict_model(best_model, data=test_input)
    y_prob = preds['prediction_score'].values
    y_pred = preds['prediction_label'].values.astype(int)
    ...
    # ── Feature importance (works for tree models; fallback to zeros) ───────
    try:
    ...
    try:
        ...
        lr_fallback = LogisticRegression(class_weight='balanced', max_iter=1000, random_state=42)
        lr_fallback.fit(X_train_s, y_train)
        explainer = shap.LinearExplainer(lr_fallback, X_train_s, feature_perturbation="interventional")

    # ── Build test loan table ───────────────────────────────────────────────
    ...
    return (best_model, scaler, list(X_train.columns), auc, prec, rec,
            ...
            results_df, model_name, X_train, X_train_s)


def call_claude_api(messages, system_prompt):
    """Call Groq API for the fraud advisor bot."""
    try:
        ...
        resp = requests.post(
            ...
            json={"model": "llama-3.3-70b-versatile", "max_tokens": 1000, "messages": groq_messages},
            timeout=30
        )
        data = resp.json()
        if "choices" in data:
...
- For what-if questions (e.g. "what if AML flag is removed"), reason through how the score would change
...
def score_application(inputs, model, scaler, feature_names, numeric_features, categorical_features, doc_flag_cols):
    ...
    preds = predict_model(model, data=app)
    return float(preds['prediction_score'].iloc[0])


def render_loan_table(data, category):
    color_map = {
        'TP': ('#34d399', '#064e3b', '✅ Correctly caught fraud',      'Model said FRAUD → Actually FRAUD'),
        'FP': ('#fbbf24', '#78350f', '⚠️ False alarm — innocent',      'Model said FRAUD → Actually CLEAN'),
    ...
    priority_cols = [
        ...
        'Broker_Involved', 'Broker_DSA_Name', 'doc_risk_score',
...
def main():
    (model, scaler, feature_names, auc, prec, rec,
     tn, fp, fn, tp, coef_df, kf_scores, df, doc_flag_cols,
     numeric_features, categorical_features, test_df, explainer,
     results_df, model_name, X_train, X_train_s) = load_and_train()

    st.markdown(f"""
    <div class="hero">
        <div class="badge">HOUSING LOAN · FRAUD DETECTION SYSTEM</div>
        <h1>Anti-Fraud Detection Dashboard</h1>
        <p>{model_name} (PyCaret Best Model) · 20-Year Dataset · 10,080 Loan Applications</p>
    </div>""", unsafe_allow_html=True)
    ...
    with tab1:
        ...
        with c1:
            st.markdown(f"""<div class="metric-card">
                ...
                <div class="meaning">Overall model quality — how well it separates fraud from clean. 1.0 = perfect.</div>
            </div>""", unsafe_allow_html=True)
        with c2:
            st.markdown(f"""<div class="metric-card">
        ...
        with col_left:
            st.markdown('<p style="font-size:0.7rem;font-weight:600;color:#8899aa;text-transform:uppercase;letter-spacing:0.1em">CONFUSION MATRIX · TEST SET</p>', unsafe_allow_html=True)
            ca, cb = st.columns(2)
            with ca:
            ...
            with cb:
                st.markdown(f"""<div class="metric-card" style="border-color:#78350f">
                    ...
                    <div class="meaning">Innocent borrowers wrongly flagged ⚠️</div>
                </div>""", unsafe_allow_html=True)
    ...
    with tab2:
        ...
        if submitted:
            ...
            try:
                shap_vals = explainer.shap_values(app_df)
    ...
    with tab3:
        ...
        st.markdown('<p style="color:#8899aa;font-size:0.9rem">Higher value = more influential. For tree models this is feature importance; for linear models it is the coefficient magnitude.</p>', unsafe_allow_html=True)
        ...
        with col_fraud:
            ...
            max_w = top_fraud['weight'].max() or 1
        ...
        with col_clean:
            st.markdown('<p style="font-size:0.7rem;font-weight:600;color:#8899aa;text-transform:uppercase;letter-spacing:0.1em">BOTTOM 10 — LEAST INFLUENTIAL</p>', unsafe_allow_html=True)
            top_clean = coef_df.tail(10).iloc[::-1]
            max_w = abs(top_clean['weight']).max() or 1
    ...
    with tab4:
        ...
        st.markdown('<p style="color:#8899aa;font-size:0.9rem">Click any category to see the real loan applications behind each prediction type.</p>', unsafe_allow_html=True)
        ...
        with b2:
            ...
            <div style="background:#78350f;border:2px solid #fbbf24;border-radius:12px;padding:1rem;text-align:center;margin-bottom:10px">
                ...
                <div style="font-size:0.72rem;color:#fde68a;margin-top:4px">Innocent flagged ⚠️</div>
            </div>""", unsafe_allow_html=True)
    ...
    # ── TAB 5: PYCARET MODEL COMPARISON ──────────────────────────────────
    with tab5:
        st.markdown("### PyCaret — All Models Compared")
        st.markdown(f'<p style="color:#8899aa;font-size:0.9rem">PyCaret tested multiple models and selected <strong style="color:#60a5fa">{model_name}</strong> as the best by AUC. Results below.</p>', unsafe_allow_html=True)
        if results_df is not None and not results_df.empty:
            st.dataframe(results_df.style.highlight_max(subset=['AUC'], color='#1e3a5f'), use_container_width=True)
        else:
