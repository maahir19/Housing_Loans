    ...
    with tab2:
        ...
        if submitted:
            ...
            if effective_foir > 0.65:
                credit_violations.append(f"Post-loan FOIR {effective_foir:.2f} — estimated EMI Rs{estimated_emi:,.0f}/mo + existing Rs{emi:,}/mo exceeds 65% of actual bank credits Rs{actual_credit:,}/mo")
            if emi_to_income > 0.55:
                credit_violations.append(f"EMI unaffordable — estimated Rs{estimated_emi:,.0f}/mo is {emi_to_income*100:.0f}% of actual bank credits Rs{actual_credit:,}/mo (stated Rs{stated_income:,}/mo cannot be verified)")
            if ltv > 0.90:
