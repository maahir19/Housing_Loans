...
def main():
    ...
    with tab2:
        ...
        if submitted:
            ...
            # Render as styled bars (no matplotlib needed)
            ...
            for _, row in contrib_df.iterrows():
                ...
                arrow     = "▲ Increases fraud risk" if direction else "▼ Reduces fraud risk"
                arrow_col = "#f87171" if direction else "#34d399"
                shap_str  = f"+{row['SHAP']:.3f}" if direction else f"{row['SHAP']:.3f}"
                rows_html += f"""
                <div style="margin-bottom:10px">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:3px">
                        <span style="font-size:0.78rem;color:#f0f4f8;font-family:monospace;max-width:60%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">{row['Feature'][:40]}</span>
                        <span style="font-size:0.75rem;color:{arrow_col};margin-left:8px;white-space:nowrap">{arrow} &nbsp;<strong>{shap_str}</strong></span>
                    </div>
                    <div style="background:#1e2533;border-radius:4px;height:8px;overflow:hidden">
                        <div style="width:{bar_width}%;height:100%;background:{bar_color};border-radius:4px;opacity:0.85"></div>
                    </div>
                </div>"""

            st.markdown(f"""
            <div style="background:#1a1f2e;border:1px solid #2d3748;border-radius:14px;padding:1.5rem 1.75rem">
                <p style="color:#8899aa;font-size:0.82rem;margin:0 0 1rem">
                    Each bar shows how much that feature <span style="color:#f87171">pushed the score up</span> or
                    <span style="color:#34d399">pulled it down</span> for this specific application.
                    Only the 12 most influential factors are shown.
                </p>
                {rows_html}
            </div>""", unsafe_allow_html=True)
