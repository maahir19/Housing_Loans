    ...
    with tab2:
        ...
        if submitted:
            ...
            with r2:
                ...
                if anomaly_notes:
                    anomaly_html = "".join([f'<div style="display:flex;align-items:flex-start;gap:8px;margin-bottom:10px"><span style="color:#fbbf24;font-size:0.9rem;margin-top:1px">!</span><span style="font-size:1rem;color:#fde68a;line-height:1.5">{a}</span></div>' for a in anomaly_notes])
                    st.markdown(f'<div style="background:#1c1100;border:1px solid #fbbf2444;border-radius:12px;padding:1rem 1.25rem;margin-top:0.5rem"><div style="font-size:0.72rem;font-weight:600;color:#fbbf24;text-transform:uppercase;letter-spacing:0.1em;margin-bottom:10px">WORTH A SECOND LOOK ({len(anomaly_notes)})</div>{anomaly_html}</div>', unsafe_allow_html=True)
