...
def render_chat_widget(inputs, score, inflate, flags_count, shap_summary):
    ...
    for msg in st.session_state.chat_history:
        ...
        st.markdown(f'<div style="background:{bg};border-radius:10px;padding:0.6rem 1rem;margin:6px 0;text-align:{align}"><span style="font-size:0.85rem;color:{color}">{msg["content"]}</span></div>', unsafe_allow_html=True)
...
def main():
    ...
    st.markdown(f"""<div class="hero"><div class="badge">HOUSING LOAN - FRAUD DETECTION SYSTEM</div>
        ...
        <p>Best Model: <strong>{best_name}</strong> - 20-Year Dataset - 10,080 Loan Applications</p></div>""", unsafe_allow_html=True)
    ...
    with tab2:
        ...
        if submitted:
            ...
            critical_flags_triggered = sum([
                ...
                disb_nonseller, pdf_anomaly, foreclosed
            ])
            ...
            # Effective FOIR after this loan
            effective_foir = total_monthly_obligation / stated_income if stated_income > 0 else 99
            # EMI as % of income
            emi_to_income = estimated_emi / stated_income if stated_income > 0 else 99

            credit_violations = []
            if foir > 0.65:
                credit_violations.append(f"FOIR {foir:.2f} — over 65% of income already committed to EMIs")
            if effective_foir > 0.65:
                credit_violations.append(f"Post-loan FOIR {effective_foir:.2f} — estimated EMI of Rs{estimated_emi:,.0f}/mo + existing Rs{emi:,}/mo exceeds 65% of income Rs{stated_income:,}/mo")
            if emi_to_income > 0.55:
                credit_violations.append(f"EMI unaffordable — estimated Rs{estimated_emi:,.0f}/mo is {emi_to_income*100:.0f}% of stated income Rs{stated_income:,}/mo")
            if ltv > 0.90:
                credit_violations.append(f"LTV {ltv:.2f} — borrower has less than 10% skin in the game")
            if (age + tenure) > 65:
                credit_violations.append(f"Age {age} + Tenure {tenure}yr = loan ends at {age+tenure} — beyond retirement age")
            if tenure < 5 and loan_amount > 2000000:
                credit_violations.append(f"Tenure {tenure}yr too short for Rs{loan_amount:,} loan — estimated EMI Rs{estimated_emi:,.0f}/mo is not realistic")
            if cibil < 650:
                credit_violations.append(f"CIBIL {cibil} — below minimum acceptable threshold of 650")
            ...
            with r2:
                ...
                # Final verdict — fraud flags + credit rules both contribute
                if score >= 0.7 or critical_flags_triggered >= 1:
                    ...
                    if critical_flags_triggered >= 1 and score < 0.7:
                        action = "Critical document flag triggered — send to fraud investigation team"
                    else:
                        action = "High fraud probability — send to fraud investigation team"
                elif credit_violations:
                    color, bg, label = "#f87171", "#1c0a0a", "REJECT — CREDIT POLICY BREACH"
                    action = credit_violations[0]
                elif score >= 0.4 or flags_count_excl_oc >= 1:
                    color, bg, label = "#fbbf24", "#1c1100", "MANUAL REVIEW REQUIRED"
                    if flags_count_excl_oc >= 1 and score < 0.4:
                        action = f"{flags_count_excl_oc} suspicious flag(s) raised — do not auto-approve"
                    else:
                        action = "Moderate fraud probability — verify documents before proceeding"
                else:
                    color, bg, label = "#34d399", "#021c0e", "PROCEED TO CREDIT APPRAISAL"
                    action = "No flags raised and low model score — safe to proceed"
                ...
                <div style="background:{bg};border:1px solid {color}44;border-radius:16px;padding:1.75rem 2rem;margin:1rem 0">
                  <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:1.5rem;flex-wrap:wrap">
                    <div style="text-align:center;flex:1;min-width:140px">
                      <div style="font-size:0.65rem;font-weight:600;color:#8899aa;text-transform:uppercase;letter-spacing:0.1em;margin-bottom:6px">MODEL SCORE</div>
                      <div style="font-size:3rem;font-weight:700;color:{score_color};line-height:1">{score*100:.1f}%</div>
                      <div style="font-size:0.75rem;color:{score_color};margin-top:4px;opacity:0.85">{score_label}</div>
                    </div>
                    ...
                    <div style="text-align:center;flex:1.4;min-width:160px">
                      <div style="font-size:0.65rem;font-weight:600;color:#8899aa;text-transform:uppercase;letter-spacing:0.1em;margin-bottom:6px">DECISION</div>
                      <div style="font-size:1.1rem;font-weight:700;color:{color};line-height:1.3">{label}</div>
                      <div style="font-size:0.78rem;color:#aab4c4;margin-top:8px;line-height:1.5">{action}</div>
                    </div>
                  </div>
                ...
                if credit_violations:
                    violations_html = "".join([f'<div style="display:flex;align-items:flex-start;gap:8px;margin-bottom:6px"><span style="color:#f87171;font-size:0.8rem;margin-top:1px">✕</span><span style="font-size:0.8rem;color:#fca5a5">{v}</span></div>' for v in credit_violations])
                    st.markdown(f'<div style="background:#1c0a0a;border:1px solid #f8717144;border-radius:12px;padding:1rem 1.25rem;margin-top:0.5rem"><div style="font-size:0.65rem;font-weight:600;color:#f87171;text-transform:uppercase;letter-spacing:0.1em;margin-bottom:10px">CREDIT POLICY VIOLATIONS — {len(credit_violations)} BREACH(ES)</div>{violations_html}</div>', unsafe_allow_html=True)
            ...
            with s4:
                ...
                st.markdown(f'<div class="metric-card"><div class="label">Est. EMI / Income</div><div class="value" style="color:{col}">{emi_to_income*100:.0f}%</div><div class="meaning">Rs{estimated_emi:,.0f}/mo EMI on Rs{stated_income:,}/mo income</div></div>', unsafe_allow_html=True)
            ...
            if not contrib_df.empty:
                ...
                st.markdown('<div style="background:#1a1f2e;border:1px solid #2d3748;border-radius:14px;padding:1.25rem 1.5rem 0.25rem"><p style="color:#8899aa;font-size:0.82rem;margin:0 0 0.75rem">Each bar shows how much that feature <span style="color:#f87171">pushed the score up</span> or <span style="color:#34d399">pulled it down</span>. Top 12 factors shown.</p></div>', unsafe_allow_html=True)
                for _, row in contrib_df.iterrows():
                    ...
                    st.markdown(f'<div style="margin:6px 0;padding:0 4px"><div style="display:flex;justify-content:space-between;margin-bottom:3px"><span style="font-size:0.78rem;color:#f0f4f8;font-family:monospace">{str(row["Feature"])[:42]}</span><span style="font-size:0.75rem;color:{ac}">{ar} <b>{ss}</b></span></div><div style="background:#1e2533;border-radius:4px;height:7px"><div style="width:{bw}%;height:100%;background:{bc};border-radius:4px"></div></div></div>', unsafe_allow_html=True)
    ...
    with tab4:
        ...
        with b1:
            st.markdown(f'<div style="background:#064e3b;border:2px solid #34d399;border-radius:12px;padding:1rem;text-align:center;margin-bottom:10px"><div style="font-size:2rem;font-weight:700;color:#34d399">{tp:,}</div><div style="font-size:0.85rem;font-weight:600;color:#34d399">True Positive</div><div style="font-size:0.72rem;color:#6ee7b7;margin-top:4px">Fraud correctly caught</div></div>', unsafe_allow_html=True)
            if st.button("View TP loans", key="btn_tp"): st.session_state['selected']='TP'
        with b2:
            st.markdown(f'<div style="background:#78350f;border:2px solid #fbbf24;border-radius:12px;padding:1rem;text-align:center;margin-bottom:10px"><div style="font-size:2rem;font-weight:700;color:#fbbf24">{fp:,}</div><div style="font-size:0.85rem;font-weight:600;color:#fbbf24">False Positive</div><div style="font-size:0.72rem;color:#fde68a;margin-top:4px">Innocent flagged</div></div>', unsafe_allow_html=True)
            if st.button("View FP loans", key="btn_fp"): st.session_state['selected']='FP'
        with b3:
            st.markdown(f'<div style="background:#7f1d1d;border:2px solid #f87171;border-radius:12px;padding:1rem;text-align:center;margin-bottom:10px"><div style="font-size:2rem;font-weight:700;color:#f87171">{fn:,}</div><div style="font-size:0.85rem;font-weight:600;color:#f87171">False Negative</div><div style="font-size:0.72rem;color:#fca5a5;margin-top:4px">Fraud missed</div></div>', unsafe_allow_html=True)
            if st.button("View FN loans", key="btn_fn"): st.session_state['selected']='FN'
        with b4:
            st.markdown(f'<div style="background:#1e3a5f;border:2px solid #60a5fa;border-radius:12px;padding:1rem;text-align:center;margin-bottom:10px"><div style="font-size:2rem;font-weight:700;color:#60a5fa">{tn:,}</div><div style="font-size:0.85rem;font-weight:600;color:#60a5fa">True Negative</div><div style="font-size:0.72rem;color:#93c5fd;margin-top:4px">Clean correctly approved</div></div>', unsafe_allow_html=True)
    ...
    with tab5:
        ...
        <div style="background:#1e3a5f;border:1px solid #1d4ed8;border-radius:12px;padding:1.25rem 1.5rem;margin-top:1rem">
            <div style="font-size:0.7rem;font-weight:600;color:#60a5fa;text-transform:uppercase;letter-spacing:0.1em;margin-bottom:0.5rem">WHAT DO THESE MEAN?</div>
            <p style="color:#cbd5e1;font-size:0.85rem;margin:0;line-height:1.8">
            <strong style="color:#f0f4f8">Individual</strong> — one model working alone<br>
            <strong style="color:#a78bfa">Voting Ensemble</strong> — XGBoost + Random Forest both vote, majority wins<br>
            <strong style="color:#67e8f9">Stacking Ensemble</strong> — base models extract patterns, Logistic Regression makes the final call
            </p>
        </div>""", unsafe_allow_html=True)
