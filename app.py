...
def main():
    ...
    with tab2:
        ...
        if submitted:
            ...
            # Critical flags that override the model score — these alone are enough to escalate
            critical_flags_triggered = sum([bank_tampered, id_altered, aml_flag, pep, sig_mismatch, overseas])
            ...
            with r2:
                if score >= 0.7 or critical_flags_triggered >= 1:
                    ...
                    if critical_flags_triggered >= 1 and score < 0.7:
                        sub = f"Critical flag(s) triggered — escalate regardless of score"
                elif score >= 0.4 or flags_count_excl_oc >= 3:
                    color,bg,label,sub = "#fbbf24","#1c1100","MODERATE RISK - REVIEW REQUIRED","Manual verification recommended"
                    if flags_count_excl_oc >= 3 and score < 0.4:
                        sub = f"{flags_count_excl_oc} suspicious flags raised — do not auto-approve"
                else:
