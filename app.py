...
def load_and_train():
    ...
    for name, clf in candidates.items():
        ...
        comparison_rows.append({'Model':name,'AUC':round(m_auc,4),'Recall':round(m_rec,4),'Precision':round(m_prec,4),'F1':round(m_f1,4)})
        if m_auc > best_auc:
    ...
    coef_df = pd.DataFrame({'feature': X_train.columns, 'weight': importances}).sort_values('weight', ascending=False)
    ...
    try:
        ...
        explainer = shap.LinearExplainer(lr, X_train_s, feature_perturbation="interventional")
        use_tree_shap = False
...
def main():
    ...
    is_tree = best_name in {'Random Forest','Gradient Boosting','Extra Trees','Decision Tree'}
    ...
    with tab2:
        ...
        if submitted:
            ...
            with r2:
                if score >= 0.7:   color,bg,label,sub = "#f87171","#1c0a0a","HIGH RISK - FLAG IMMEDIATELY","Send to fraud investigation team"
                elif score >= 0.4: color,bg,label,sub = "#fbbf24","#1c1100","MODERATE RISK - REVIEW REQUIRED","Manual verification recommended"
                else:              color,bg,label,sub = "#34d399","#021c0e","LOW RISK - PROCEED","Proceed to credit appraisal"
                st.markdown(f'<div style="background:{bg};border:1px solid {color}33;border-radius:16px;padding:2rem;text-align:center;margin:1rem 0"><div style="font-size:3.5rem;font-weight:700;color:{color};line-height:1">{score*100:.1f}%</div><div style="font-size:1rem;font-weight:600;color:{color};margin-top:8px">{label}</div><div style="font-size:0.85rem;color:#8899aa;margin-top:4px">{sub}</div></div>', unsafe_allow_html=True)

            inflate     = min(stated_income/actual_credit if actual_credit else 1, 10)
            flags_count = sum([bank_tampered,pdf_anomaly,aml_flag,pep,disb_nonseller,sig_mismatch,id_altered,itr_before,overseas,month_end_park,val_inflation,salary_personal,pf_missing,unreachable,foreclosed,round_salary,email_mismatch])
            ...
            with s2:
                ...
                st.markdown(f'<div class="metric-card"><div class="label">Fraud Flags Triggered</div><div class="value" style="color:{col}">{flags_count} / 17</div><div class="meaning">Number of suspicious flags</div></div>', unsafe_allow_html=True)
            with s3:
            ...
            try:
                if use_tree_shap:
                    sv = explainer.shap_values(app_df)
    ...
    with tab3:
        ...
        with col_clean:
            ...
            for _, row in bot10.iterrows():
                pct = int(abs(row['weight'])/mw*100)
                st.markdown(f'<div style="margin-bottom:10px"><div style="display:flex;justify-content:space-between;margin-bottom:3px"><span style="font-size:0.78rem;color:#f0f4f8;font-family:monospace">{row["feature"][:38]}</span><span style="font-size:0.78rem;font-weight:600;color:#34d399">{row["weight"]:.3f}</span></div><div style="background:#1e2533;border-radius:4px;height:7px;overflow:hidden"><div style="width:{pct}%;height:100%;background:linear-gradient(90deg,#059669,#34d399);border-radius:4px"></div></div></div>', unsafe_allow_html=True)
    ...
    with tab5:
        st.markdown("### Model Comparison - All 6 Models Tested")
        st.markdown(f'<p style="color:#8899aa;font-size:0.9rem">All models were trained on your data. <strong style="color:#60a5fa">{best_name}</strong> won with the highest AUC.</p>', unsafe_allow_html=True)
        st.dataframe(comparison_df, use_container_width=True, hide_index=True)
        max_auc = comparison_df['AUC'].max()
        for _, row in comparison_df.iterrows():
            bw      = int(row['AUC']/max_auc*100)
            is_best = row['Model'] == best_name
            color   = "#60a5fa" if is_best else "#4b5563"
            badge   = " BEST" if is_best else ""
            st.markdown(f'<div style="margin-bottom:10px"><div style="display:flex;justify-content:space-between;margin-bottom:3px"><span style="font-size:0.85rem;color:#f0f4f8;font-weight:{"600" if is_best else "400"}">{row["Model"]}{badge}</span><span style="font-size:0.85rem;color:{color};font-weight:600">AUC {row["AUC"]}</span></div><div style="background:#1e2533;border-radius:4px;height:10px;overflow:hidden"><div style="width:{bw}%;height:100%;background:{color};border-radius:4px"></div></div></div>', unsafe_allow_html=True)
