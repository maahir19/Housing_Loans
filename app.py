...
st.set_page_config(page_title="Housing Loan Fraud Detection", page_icon="🏦", layout="wide")
...
def render_loan_table(data, category):
    ...
    show_cols = ['Application_ID','Age','Gender','Employment_Type',
                 'Monthly_Income_Stated','Actual_Bank_Credit_Monthly',
                 'income_inflate_ratio','CIBIL_Score','Loan_Amount_INR',
    ...
    rename = {
        ...
        'Monthly_Income_Stated'    : 'Stated Income',
        'Actual_Bank_Credit_Monthly': 'Actual Credits',
        'income_inflate_ratio'     : 'Inflate Ratio',
        'CIBIL_Score'              : 'CIBIL',
    ...
    if 'Stated Income'  in display.columns: display['Stated Income']  = display['Stated Income'].apply(lambda x: f"₹{x:,.0f}")
    if 'Actual Credits' in display.columns: display['Actual Credits'] = display['Actual Credits'].apply(lambda x: f"₹{x:,.0f}" if pd.notna(x) else "—")
    if 'Loan Amount'    in display.columns: display['Loan Amount']    = display['Loan Amount'].apply(lambda x: f"₹{x:,.0f}")
    if 'Inflate Ratio'  in display.columns: display['Inflate Ratio']  = display['Inflate Ratio'].apply(lambda x: f"{x:.2f}x")
    if 'Risk Score %'   in display.columns: display['Risk Score %']   = display['Risk Score %'].apply(lambda x: f"{x}%")

    st.dataframe(display.reset_index(drop=True), use_container_width=True, height=400)
    st.caption(f"Showing {len(display):,} loan applications in this category")
    ...
    with tab4:
        ...
        if selected:
            ...
            show_cols = ['Application_ID','Age','Gender','Employment_Type',
                         'Monthly_Income_Stated','Actual_Bank_Credit_Monthly',
                         'income_inflate_ratio','CIBIL_Score','Loan_Amount_INR',
                         'Fraud_Type','risk_score','category']
            show_cols = [c for c in show_cols if c in data_map[selected].columns]
            csv = data_map[selected][show_cols].to_csv(index=False)
            st.download_button(
                label=f"⬇️ Download {selected} loans as CSV",
                data=csv,
