        ...
        if submitted:
            ...
            with r2:
                if score >= 0.7 or critical_flags_triggered >= 1:
                    # High model score OR any critical flag = HIGH RISK
                    ...
                    if critical_flags_triggered >= 1 and score < 0.7:
                        sub = f"Critical flag triggered — escalate regardless of model score"
                elif score >= 0.4 or flags_count_excl_oc >= 1:
                    # Medium model score OR even 1 flag = MODERATE RISK — never auto-approve with any flag active
                    color,bg,label,sub = "#fbbf24","#1c1100","MODERATE RISK - REVIEW REQUIRED","Manual verification required — do not auto-approve"
                    if flags_count_excl_oc >= 1 and score < 0.4:
                        sub = f"{flags_count_excl_oc} suspicious flag(s) raised — manual review required before proceeding"
                else:
                    # Only PROCEED if model score is low AND zero flags raised
                    color,bg,label,sub = "#34d399","#021c0e","LOW RISK - PROCEED","No flags raised — proceed to credit appraisal"
